...
export default function ListView({ items, loading, onChanged, mode }: Props) {
  ...
  // Filter: Aktiv zeigt nur "geplant", Archiv zeigt alle anderen
        ...
        return (
          <article key={a.id} className={`p-3 rounded-2xl ${past ? 'bg-gray-100/70' : 'bg-white'} text-black`}>
            ...
            <div className="mt-2 grid grid-cols-2 gap-2">
              <button className="p-2 bg-black text-white" onClick={() => mutate(a.id, { status: 'abgeschlossen' })}>
                Als abgeschlossen
              </button>
              <button className="p-2 bg-gray-800 text-white" onClick={() => mutate(a.id, { status: 'nicht_erschienen' })}>
                Nicht erschienen
              </button>
              <button className="p-2 bg-gray-700 text-white" onClick={() => mutate(a.id, { status: 'kurzfristig_abgesagt' })}>
                Kurzfr. abgesagt
              </button>
              <button
              ...
              <button className="col-span-2 p-2 bg-white border" onClick={() => remove(a.id)}>
                Löschen
              </button>
            </div>
          </article>
