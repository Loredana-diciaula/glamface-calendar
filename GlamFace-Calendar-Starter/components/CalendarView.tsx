...
export default function CalendarView({
  items, loading, mode, onEdit, onCreate, onChanged, readonly = false,
  ...
  const changeMonth = (delta: number) => {
    const m = new Date(month); m.setMonth(m.getMonth()+delta); setMonth(m); setSelected(null)
  }

  const archive = async (id: string, e?: React.MouseEvent) => {
    e?.stopPropagation()
    if (onArchive) await onArchive(id); else await supabase.from('appointments').update({ archived_at: new Date().toISOString() }).eq('id', id)
    onChanged()
  }
  const restore = async (id: string, e?: React.MouseEvent) => {
    e?.stopPropagation()
    if (onRestore) await onRestore(id); else await supabase.from('appointments').update({ archived_at: null }).eq('id', id)
    onChanged()
  }
  const hardDelete = async (id: string, e?: React.MouseEvent) => {
    ...
    if (onHardDelete) await onHardDelete(id); else await supabase.from('appointments').delete().eq('id', id)
    onChanged()
  }
  ...
  return (
    <section className="space-y-2">
      ...
      <div className="flex items-center justify-between">
        <button className="p-2" onClick={()=>changeMonth(-1)}>←</button>
              ...
              >
                ...
                {!readonly ? (
                  <div className="flex gap-2">
                    <button className="px-3 py-1 rounded-2xl border" onClick={(e)=>{e.stopPropagation(); onEdit(a)}}>Bearbeiten</button>
                    {mode === 'active' ? (
                      <button className="px-3 py-1 rounded-2xl bg-black text-white" onClick={(e)=>archive(a.id, e)}>Archivieren</button>
                    ) : (
                      <>
                        <button className="px-3 py-1 rounded-2xl bg-black text-white" onClick={(e)=>restore(a.id, e)}>Wiederherstellen</button>
                        <button className="px-3 py-1 rounded-2xl bg-red-600 text-white" onClick={(e)=>hardDelete(a.id, e)}>Endgültig löschen</button>
                      </>
                    )}
                  </div>
                ) : (
